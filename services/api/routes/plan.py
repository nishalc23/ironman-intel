...
def get_split(athlete_id: int = Depends(current_athlete_id)):
...
def get_today_plan(gym: bool = True, discipline: str | None = None, split: str | None = None, athlete_id: int = Depends(current_athlete_id)):
    with get_db() as db:
        athlete = load_athlete(db, athlete_id)
...
def get_weekly_plan(athlete_id: int = Depends(current_athlete_id)):
...
def get_adaptive_plan(week_number: int, athlete_id: int = Depends(current_athlete_id)):
    """
...
async def trigger_adaptive_generation(week_number: int | None = None, background_tasks: __import__("fastapi").BackgroundTasks = None, athlete_id: int = Depends(current_athlete_id)):
    """
